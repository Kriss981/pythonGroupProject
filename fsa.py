...
class FSA:
    ...
    def write(self, filename=None):
        ...
        nodes = set()  # set of nodes already handled

        for key, values in self.transitions.items():
            s1 = key[0]
            left_label = key[1]
            if s1 in self.accepting and s1 not in nodes:  # activating doublecircle
                dot.attr('node', shape='doublecircle')
            if s1 not in nodes:
                dot.node(str(s1), str(s1))  # adding nodes
                nodes.add(s1)
                if s1 in self.accepting:  # deactivation doublecircle
                    dot.attr('node', shape='circle')
            for v in values:
                s2 = v
                right_label = left_label
                if s2 in self.accepting and s2 not in nodes:  # activating doublecircle
                    dot.attr('node', shape='doublecircle')
                if s2 not in nodes:
                    dot.node(str(s2), str(s2))  # adding nodes
                    nodes.add(s2)
                    if s2 in self.accepting:  # deactivation doublecircle
                        dot.attr('node', shape='circle')
                if left_label != right_label:  # adding labels to edges
                    dot.edge(str(s1), str(s2), label=left_label + ":" + right_label)
                else:
                    dot.edge(str(s1), str(s2), label=left_label)
